...
export class GlobalHttpInterceptor implements HttpInterceptor {
  ...
  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    if (req.url.replace(env.backendBaseUrl, '').startsWith('/api/')) {
      ...
      if (this.isTokenBasedAuthEntryPoint(req.url)) {
        if (!this.authService.getJwtToken() && !this.authService.refreshTokenPending()) {
          return this.handleResponseError(req, next, new HttpErrorResponse({ error: { message: 'Unauthorized!' }, status: 401 }));
        } else if (!this.authService.isJwtTokenValid()) {
  ...
  private handleResponseError(req: HttpRequest<any>, next: HttpHandler, errorResponse: HttpErrorResponse): Observable<HttpEvent<any>> {
    ...
    } else if (!req.url.replace(env.backendBaseUrl, '').startsWith('/api/plugins/rpc')) {
  ...
  private isInternalUrlPrefix(url): boolean {
    for (const index in this.internalUrlPrefixes) {
      if (url.replace(env.backendBaseUrl, '').startsWith(this.internalUrlPrefixes[index])) {
        return true;
      }
    }
    return false;
  ...
  private isTokenBasedAuthEntryPoint(url): boolean {
    return url.replace(env.backendBaseUrl, '').startsWith('/api/') &&
      !url.replace(env.backendBaseUrl, '').startsWith(Constants.entryPoints.login) &&
      !url.replace(env.backendBaseUrl, '').startsWith(Constants.entryPoints.tokenRefresh) &&
      !url.replace(env.backendBaseUrl, '').startsWith(Constants.entryPoints.nonTokenBased);
  }
