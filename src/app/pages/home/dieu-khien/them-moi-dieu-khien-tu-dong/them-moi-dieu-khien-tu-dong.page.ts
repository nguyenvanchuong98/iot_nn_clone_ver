...
import { ThemMoiSuKienModalComponent } from './them-moi-su-kien-modal/them-moi-su-kien-modal.component';
import { Device } from 'src/app/shared/models/device-model';
import { GetGatewayResponse } from 'src/app/shared/models/gateway-model';
import { ThemMoiRpcModalComponent } from './them-moi-rpc-modal/them-moi-rpc-modal.component';
...
import { ThemMoiDieuKienThietBiModalComponent } from './them-moi-dieu-kien-thiet-bi-modal/them-moi-dieu-kien-thiet-bi-modal.component';
...
@Component({
  selector: 'app-them-moi-dieu-khien-tu-dong',
  templateUrl: './them-moi-dieu-khien-tu-dong.page.html',
  styleUrls: ['./them-moi-dieu-khien-tu-dong.page.scss'],
})
export class ThemMoiDieuKhienTuDongPage implements OnInit {
  damTomId: string = null;
  damTom: DamtomFull = null;
  damTomName: string = null;
  isExistName: boolean;
  isGroupRpcControl: boolean;
  // xayRaDongThoi: boolean = true;
  // xayRaDongThoiTB: boolean = true;
  suKien: SuKienDto[] = [];
  ...
  ngOnInit() {
    this._activatedRoute.queryParams.subscribe((params) => {
      ...
      this.fetchingDamTomData();
      this.getDeviceZone();
    });
  }

  async fetchingDamTomData() {
    this.isLoadingInit = true;

    // trungdt - lấy dữ liệu đầm tôm
    try {
      this.damTom = await this._damTomService
        .getDamtomByIdAll(this.damTomId)
        .toPromise();
      this.damTomName = this.damTom?.name;
    } catch (error) {}
    // trungdt - lấy danh sách nhóm điều khiển của đầm
    try {
      this.groupRpcs = await this._dieuKhienService
        .getAllGroupRpc(this.damTomId)
        .toPromise();
    } catch (error) {}
  ...
  validate(fieldName: string, errorName: string) {
    return (
      ...
      this.form.controls[fieldName].hasError(errorName)
    );
  }
  ...
  isFormValid(): boolean {
    return (
      !!this.form &&
      this.form.valid &&
      this.dieuKhien.length > 0 &&
      (this.suKien.length > 0 || this.dieuKienTB.length > 0) &&
      !this.checkValidVaoCacNgay() &&
      !this.checkValidTime()
    );
  }
  ...
  getRpcLabel(tenThietBi: string) {
    const device = this.lstAllDvice.RPC.find((el) => {
      return el.name === tenThietBi;
    });
    return device?.label ? device?.label : device?.name;
  }
  ...
  async submit() {
    ...
    for (let i = 0; i < this.dieuKhien.length; i++) {
      if (this.dieuKhien[i].typeRpc === 'group-rpc') {
        ...
        rpcAlarm.deviceProfileAlarm.dftAlarmRule.groupRpcIds.push(
          this.dieuKhien[i].groupRpcId
        );
        continue;
      }
      var rpcSetting: RpcSettingDto;
      if (this.dieuKhien[i].deviceType !=='REM') {
        // gọi api tạo mới với điều khiển thiết bị
        rpcSetting = {
        ...
        rpcSetting = {
          ...
          remId: this.dieuKhien[i].deviceId,
          // timeRem: this.getRemByIdRem(this.dieuKhien[i].deviceId).finishTime,
      ...
      try {
        ...
        rpcAlarm.deviceProfileAlarm.dftAlarmRule.rpcSettingIds.push(
          response.id
        );
      } catch (error) {
    ...
    this.suKien.forEach((el, i) => {
      ...
      rpcAlarm.deviceProfileAlarm.createRules.MAJOR.condition.condition.push(
        obj
      );
    });
    ...
    this.dieuKienTB.forEach((el) => {
      ...
      const obj: AlarmRule = {
        ...
        predicate: {
          ...
          operation: 'EQUAL',
          value: {
            defaultValue: el.deviceType === 'REM' ? 0 : el.trangThai ? 1 : 0,
          },
        },
      };
    ...
    try {
      const response = await this._damTomRpcAlarmService
        .saveAlarm(rpcAlarm)
        .toPromise();
      ...
      this.isExistName =
        error.error === 'Tên kịch bản đã tồn tại!' ? true : false;
      // this._toastCtl
      //   .create({
      //     message:
      //       error.error === 'Tên điều khiển tự động đã tồn tại!'
  ...
  getRemByIdRem(idRem: string): SpecialDevice {
    var rem: SpecialDevice;
      this.deviceZones.forEach(zone => {
        if (!!zone.rpcRemList.find(rem => rem.id === idRem)) {
          rem = zone.rpcRemList.find(rem => rem.id === idRem);
        }
      })
    return rem;
  ...
  checkValidTime() {
    return (
      this.toMillis(this.form.get('scheduleStart').value) >
      this.toMillis(this.form.get('scheduleEnd').value)
    );
  }
  ...
  convertLongToDate(time: string) {
    // let time = new Date(MILISECONDS).toISOString().substr(11, 8).toString();
    time.split(':');
